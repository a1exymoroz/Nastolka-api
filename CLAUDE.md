...
4. Every PR must link to the GitHub issue(s) it addresses. Include a closing keyword (e.g. `Closes #12`, `Fixes #7`) in the PR body so the issue auto-closes on merge. If no related issue exists yet, create one first before opening the PR.
5. Every issue must be added to the **Nastolka** GitHub project (owner `a1exymoroz`, project number 3): `gh project item-add 3 --owner a1exymoroz --url <issue-url>`.
