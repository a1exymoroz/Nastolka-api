...
public interface HistoryPlayerRepository extends JpaRepository<HistoryPlayer, Long> {
    ...
    @Query("select p.user.username as username, p.user.displayName as displayName, count(p) as gamesPlayed, " +
            ...
            "group by p.user.id, p.user.username, p.user.displayName")
